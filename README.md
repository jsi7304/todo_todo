...
<ol>
  ...
  <p><h5>개요</h5>- 데이터를 불러오는 시간을 기다리지 못하고 undefined 오류가 발생</p>
  <p><h5>해결과정</h5>- 데이터가 없을때에 대체 데이터를 넣어주면 된다는 것을 확인</p>
  <p><h5>해결</h5>- if문으로 데이터가 없을 시에는 빈 div를 출력시킴으로써, 데이터가 없을때에는 빈 div가,</p>
  <p>데이터가 들어왔을 때에는 해당 데이터가 정상적으로 출력되는 것을 확인</p>
  </p>
</ol>
